function q1
	...
	for meth = 1:2
		...
		LinePlot(S, U(end, :), U(1, :), 'Cost of option at t = 0', 'Cost of option at t = T', 'S', 'u(S, t)', sprintf('Crank-Nicolson using %s method', Methods(meth, :)), ['q1_' num2str(i)])
        SurfPlot(S, Time, U, 'S', 't', 'u(S,t)', sprintf('Crank-Nicolson using %s method', Methods(meth, :)), ['q1_' num2str(i)]);
        i = i+1;
	end
end
